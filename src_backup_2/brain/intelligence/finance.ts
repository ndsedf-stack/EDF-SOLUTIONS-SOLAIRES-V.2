...
export function computeFinancialStats(
  ...
  // ✅ CA Annulable Pur (Signé && !contract_secured && !waiting_deposit_logic)
  const cancellable = signedStudies.filter((s) => 
    !s.contract_secured && 
    !( !s.deposit_paid && s.has_deposit )
  );
