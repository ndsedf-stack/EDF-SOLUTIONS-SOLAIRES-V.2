...
import { logDecision } from "./decision/memory";
...
export function buildSystemBrain(studies: Study[]): Metrics & { financialStats: any } {
  ...
  const tensionLevel = computeTensionLevel(warRoomBase as any, cashAtRisk);
  ...
  const priorityCase = computePriorityCase(warRoomWithPrediction as any);
  const priorityActions = computePriorityActions(warRoomWithPrediction as any);
  ...
  // =========================
  // 5. TEMPÉRATURE COMPORTEMENTALE
  // =========================
  const labeledWarRoom = warRoomWithPrediction.map((s) => {
    const risk = computeBehavioralRisk(s);
    return { ...s, risk };
  });

  const muets = labeledWarRoom.filter((s) => s.risk === "muet");
  const agites = labeledWarRoom.filter((s) => s.risk === "agite");
  const interesses = labeledWarRoom.filter((s) => s.risk === "interesse");
  const fatigues = labeledWarRoom.filter((s) => s.risk === "fatigue");
  ...
  if (priorityCase) {
    logDecision({
      ...
      studyId: priorityCase.id,
      date: new Date().toISOString(),
      dangerScore: priorityCase.dangerScore,
      behavior: priorityCase.behavior,
      recommendedAction: priorityCase.action,
      systemState,
      tensionLevel,
      outcome: "PENDING",
    });
