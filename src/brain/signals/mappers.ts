...
export function getBehavioralState(study: Study): {
  ...
  if (risk === "muet") return { state: "MUET", icon: "🧊", label: "SILENCE" };
  if (risk === "cas_limite")
...
export function mapStudyToDisplay(s: any, st: any, antiAnnul: Record<string, any> = {}, postRefus: Record<string, any> = {}): Study {
  ...
  // Récupération des valeurs (PRIORITÉ à total_price)
  const totalCost = s.total_price || s.install_cost || s.study_data?.installCost || 0;
  const apport = s.cash_apport ?? s.study_data?.cashApport ?? 0;

  // Calcul automatique du mode selon l'apport
  let mode: "cash" | "financing" | null;
  let financingMode: "full_financing" | "with_deposit" | "cash_payment" | "partial_financing" | null;

  if (apport >= totalCost && totalCost > 0) {
    // Apport = Total → CASH obligatoire
    mode = "cash";
    financingMode = "cash_payment";
  } else if (apport > 0 && apport < totalCost) {
  ...
  // Override si valeurs déjà présentes en DB (ne pas écraser)
  if (s.payment_mode) mode = s.payment_mode;
  ...
  const isDepositLateCalc = (() => {
    ...
    return daysSinceSigned !== null && daysSinceSigned > 0;
  })();
  ...
  const mappedStudy: Study = {
    ...
    // ✅ Brain Revenue Calculation
    revenue_secured: s.status === "signed" ? totalCost : 0,
    revenue_projected: totalCost,
    revenue_gap: s.status !== "signed" ? totalCost : 0,
    // Champs calculés
    views,
    clicks,
  ...
  mappedStudy.dangerScore = basicDangerScore;
